...
model ConstrainVariableAfterTimer
  ...
  parameter SIunits.Time timeToBeginConstraint=1.0 "Variable is constrained after and excluding this time" annotation(
    Dialog(group = "Characteristics"));
...
initial equation
  ...
  triggeredSampler.trigger=false;
  
  if use_u_targetVal == false then
    tgtValue_paramInput = triggeredSampler.y;
  elseif use_u_targetVal == true then
    u_targetValue = triggeredSampler.y;
  end if;

equation
  
  if(time<=timeToBeginConstraint)then
    triggeredSampler.trigger=false;
  else
...
********************************************************/
  ...
  connect(u_variable, triggeredSampler.u) annotation(
