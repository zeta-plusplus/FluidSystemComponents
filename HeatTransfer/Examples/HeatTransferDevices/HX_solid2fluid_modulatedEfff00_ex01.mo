...
model HX_solid2fluid_modulatedEfff00_ex01
  ...
  Modelica.Blocks.Sources.Ramp ramp1(duration = 5, height = 0.2, offset = 0.7, startTime = 5) annotation(
  ...
  Sensor.DispColorGrad00 dispColorGrad00(valMin = TcontourMin, valMax = TcontourMax)  annotation(
    Placement(transformation(origin = {-23, -6}, extent = {{7, -4}, {-7, 4}})));
...
equation
  connect(boundary.ports[1], HX.port_1) annotation(
    Line(points = {{-60, 50}, {-48, 50}, {-48, 18}, {-26, 18}}, color = {0, 127, 255}, thickness = 3));
  connect(HX.port_2, pipe.port_a) annotation(
    Line(points = {{0, 18}, {4, 18}, {4, 50}, {40, 50}}, color = {0, 127, 255}, thickness = 3));
  connect(pipe.port_b, boundary1.ports[1]) annotation(
    Line(points = {{60, 50}, {80, 50}}, color = {0, 127, 255}, thickness = 3));
  ...
  connect(HX.port_solid, fixedTemperature.port) annotation(
    Line(points = {{-13, 3}, {-13, -30}}, color = {191, 0, 0}, thickness = 3));
  ...
  connect(temperatureSensor.T, dispColorGrad00.u) annotation(
    Line(points = {{-4, -21}, {-4, -6}, {-15, -6}}, color = {0, 0, 127}));
  annotation(
