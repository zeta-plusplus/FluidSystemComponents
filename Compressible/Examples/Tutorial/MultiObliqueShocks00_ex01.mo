...
model MultiObliqueShocks00_ex01
  ...
  Interfaces.FluidWithMach2portMn FluidStatMn2port(redeclare package Medium = Fluid1) annotation(
    Placement(transformation(origin = {92, -11}, extent = {{-2, -10}, {2, 10}})));
  ...
  Modelica.Fluid.Sources.MassFlowSource_T FlEnd(redeclare package Medium = Fluid1, m_flow = -1, nPorts = 1) annotation(
    Placement(transformation(origin = {120, -3}, extent = {{10, -10}, {-10, 10}})));
...
equation
  ...
  connect(FluidStatMn2port.port, FlEnd.ports[1]) annotation(
    Line(points = {{94, -3}, {110, -3}}, color = {0, 127, 255}, thickness = 3));
  ...
  connect(Shock3.portStatWithMn_2, FluidStatMn2port.portStatWithMn) annotation(
    Line(points = {{60, -11}, {90, -11}}, color = {0, 127, 255}, thickness = 3));
