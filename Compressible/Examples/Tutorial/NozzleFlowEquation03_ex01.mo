...
model NozzleFlowEquation03_ex01
  ...
  Modelica.Fluid.Sources.Boundary_pT BC_L_2(redeclare package Medium = fluid1, T = 600, nPorts = 1, p = 100 * 1000, use_p_in = true) annotation(
  ...
  Modelica.Fluid.Valves.ValveCompressible valveCompressible(redeclare package Medium = fluid1, Av = 0.8 * Modelica.Constants.pi / 4 * 0.05 ^ 2, CvData = Modelica.Fluid.Types.CvTypes.Av, dp_nominal = 100 * 1000, m_flow_nominal = 0.1, p_nominal = 2 * 100 * 1000)  annotation(
...
equation
  ...
  connect(BC_L_2.p_in, ramp_pL_2.y) annotation(
    Line(points = {{62, -46}, {80, -46}}, color = {0, 0, 127}));
