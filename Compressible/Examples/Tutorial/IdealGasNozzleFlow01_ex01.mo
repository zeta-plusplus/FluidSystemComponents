...
model IdealGasNozzleFlow01_ex01
  ...
  //-------------------------
  Sources.Boundary_pT_colorP boundary1(nPorts = 2, p = 150*1000, T(displayUnit = "K") = 400, redeclare package Medium = fluid1, valMin = pContourMin, valMax = pContourMax, sigDigits = digitP, use_p_in = true) annotation(
    Placement(transformation(origin = {-22, 42}, extent = {{-10, -10}, {10, 10}})));
  Sources.Boundary_pT_colorP boundary2(redeclare package Medium = fluid1, T (displayUnit = "K")= 288.15, nPorts = 2, p = 100*1000, valMin = pContourMin, valMax = pContourMax, sigDigits = digitP, use_p_in = true) annotation(
    Placement(transformation(origin = {-22, -42}, extent = {{-10, -10}, {10, 10}})));
  ...
  FluidSystemComponents.Compressible.Components.NozzleFlowEquation03 Restriction_1(AmechTot_par = 1000*1e-6, redeclare package Medium = fluid1) annotation(
    Placement(transformation(origin = {18, 4}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
equation
  ...
  connect(boundary1.ports[2], Restriction_1.port_a) annotation(
    Line(points = {{-12, 42}, {18, 42}, {18, 14}}, color = {0, 127, 255}));
  connect(boundary2.ports[2], Restriction_1.port_b) annotation(
    Line(points = {{-12, -42}, {18, -42}, {18, -6}}, color = {0, 127, 255}));
  annotation(
