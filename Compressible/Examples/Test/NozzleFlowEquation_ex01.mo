...
model NozzleFlowEquation_ex01
  ...
  FluidSystemComponents.Compressible.Components.NozzleFlowEquation01 flowPath(redeclare package Medium = gas1, AmechTot_par = 0.001) annotation(
    Placement(visible = true, transformation(origin = {0, 10}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
  FluidSystemComponents.Compressible.Components.NozzleFlowEquation01 flowPath1(redeclare package Medium = gas1, AmechTot_par = 0.001) annotation(
...
equation
  ...
  connect(BC_H_1.ports[1], flowPath.port_1) annotation(
    Line(points = {{-52, 10}, {-20, 10}}, color = {0, 127, 255}));
  connect(flowPath.port_2, BC_L_1.ports[1]) annotation(
    Line(points = {{20, 10}, {52, 10}}, color = {0, 127, 255}));
  connect(BC_H_2.ports[1], flowPath1.port_1) annotation(
    Line(points = {{-52, -30}, {-20, -30}}, color = {0, 127, 255}));
  connect(BC_L_2.ports[1], flowPath1.port_2) annotation(
    Line(points = {{52, -30}, {20, -30}}, color = {0, 127, 255}));  protected   annotation(
