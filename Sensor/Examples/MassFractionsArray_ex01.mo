...
model MassFractionsArray_ex01
  ...
  FluidSystemComponents.Sensor.MassFractionsArray massFractionsArray(redeclare package Medium = fluid1) annotation(
...
equation
  ...
  connect(Orifice.port_2, massFractionsArray.port) annotation(
    Line(points = {{0, 20}, {30, 20}}, color = {0, 127, 255}));
  connect(massFractionsArray.port, boundary1.ports[1]) annotation(
    Line(points = {{30, 20}, {60, 20}}, color = {0, 127, 255}));
  annotation(
    experiment(StartTime = 0, StopTime = 50, Tolerance = 1e-06, Interval = 0.1),
    __OpenModelica_simulationFlags(lv = "LOG_STATS", outputFormat = "mat", s = "dassl"));
end MassFractionsArray_ex01;
