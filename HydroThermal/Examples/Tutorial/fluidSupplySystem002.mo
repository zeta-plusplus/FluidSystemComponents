...
model fluidSupplySystem002
  ...
  //----------
  FluidSystemComponents.HydroThermal.Components.PumpConstQflow_ideal pumpConstQflow_ideal1(redeclare package Medium = liquid1, qFlowDes = 0.2) annotation(
    Placement(visible = true, transformation(origin = {-10, 10}, extent = {{-10, 10}, {10, -10}}, rotation = 90)));
  Modelica.Fluid.Sources.Boundary_pT boundary(redeclare package Medium = liquid1, T = 288.15, nPorts = 2, p = 101.3 * 1000) annotation(
  ...
  Modelica.Fluid.Valves.ValveLinear valveLinear1(redeclare package Medium = liquid1,dp_nominal(displayUnit = "Pa") = 5 * 101.3 * 1000, m_flow_nominal = 100)  annotation(
    Placement(visible = true, transformation(origin = {70, -20}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  Modelica.Fluid.Vessels.ClosedVolume volume(redeclare package Medium = liquid1,T_start = 288.15,V = 0.010, nPorts = 3, p_start = 101.3 * 1000, use_portsData = false)  annotation(
  ...
  FluidSystemComponents.HydroThermal.Components.CheckValveSpringLoad_Linear checkValveSpringLoad_Linear1(redeclare package Medium = liquid1, deltapCrack = 550 * 1000, gradientQP = 0.001 * 200 / (100 * 1000)) annotation(
...
equation
  connect(boundary.ports[2], checkValveSpringLoad_Linear1.port_2) annotation(
    Line(points = {{-10, -30}, {20, -30}, {20, 0}, {20, 0}}, color = {0, 127, 255}));
  connect(volume.ports[3], checkValveSpringLoad_Linear1.port_1) annotation(
    Line(points = {{-10, 40}, {20, 40}, {20, 20}, {20, 20}}, color = {0, 127, 255}));
