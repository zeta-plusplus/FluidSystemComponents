...
model SteamCondenser_002
  ...
  inner Modelica.Fluid.System system annotation(
    Placement(visible = true, transformation(extent = {{-140, 62}, {-120, 82}}, rotation = 0)));
  ...
  Modelica.Blocks.Math.Gain fracLiquid(k = 1 / condenser.V_t) annotation(
    Placement(visible = true, transformation(origin = {-2, 29}, extent = {{-5, -5}, {5, 5}}, rotation = 90)));
...
equation
  ...
  connect(ramp_Q_flow_in.y, furnace.Q_flow) annotation(
    Line(points = {{-7, -50}, {2, -50}, {2, -38}, {1, -38}}, color = {0, 0, 127}));
  connect(ramp_valveopen.y, SteamValve.opening) annotation(
    Line(points = {{-101, -30}, {-90, -30}, {-90, -8}}, color = {0, 0, 127}));
  ...
  connect(condenser.V, fracLiquid.u) annotation(
    Line(points = {{-2, 11}, {-2, 23}}, color = {0, 0, 127}));
  connect(fracLiquid.y, feedback.u2) annotation(
    Line(points = {{-2, 34.5}, {-2, 52}}, color = {0, 0, 127}));
  connect(ramp_r_liquidLevel.y, feedback.u1) annotation(
    Line(points = {{-27, 60}, {-10, 60}}, color = {0, 0, 127}));
  connect(pi.u, feedback.y) annotation(
    Line(points = {{20, 60}, {7, 60}}, color = {0, 0, 127}));
  ...
  connect(pump.ports[1], massFlowWater.port_b) annotation(
    Line(points = {{60, 0}, {42, 0}}, color = {0, 127, 255}));
  connect(temperatureWater.port, massFlowWater.port_b) annotation(
    Line(points = {{52, 10}, {52, -0.5}, {42, -0.5}, {42, 0}}, color = {0, 127, 255}));
  connect(massFlowWater.port_a, condenser.port_a) annotation(
    Line(points = {{22, 0}, {12, 0}}, color = {0, 127, 255}));
  ...
  connect(limiter.u, pi.y) annotation(
    Line(points = {{60, 60}, {43, 60}}, color = {0, 0, 127}));
  connect(limiter.y, pump.m_flow_in) annotation(
    Line(points = {{83, 60}, {91.5, 60}, {91.5, 8}, {80, 8}}, color = {0, 0, 127}));
  annotation(
