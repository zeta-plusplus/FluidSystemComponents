...
model NozzleFlowEquation_IdealConstAir_ex01
  extends Modelica.Icons.Example;
  //---------------
  package gas1 = FluidSystemComponents.Media.SingleGas.IdealConstAirNASAGlennModel;
  ...
  CommonAnyFluid.Components.ClosedVolumeColorP Vol(redeclare package Medium = gas1, V = 0.01, nPorts = 2, valMin = pMinVis, valMax = pMaxVis)  annotation(
    Placement(transformation(origin = {-40, 20}, extent = {{-10, -10}, {10, 10}})));
  CommonAnyFluid.Components.ClosedVolumeColorP Vol1(redeclare package Medium = gas1, V = 0.01, nPorts = 2, valMin = pMinVis, valMax = pMaxVis) annotation(
    Placement(transformation(origin = {40, 20}, extent = {{-10, -10}, {10, 10}})));
  CommonAnyFluid.Components.ClosedVolumeColorP Vol2(redeclare package Medium = gas1, V = 0.01, nPorts = 2, valMin = pMinVis, valMax = pMaxVis) annotation(
    Placement(transformation(origin = {-40, -34}, extent = {{-10, -10}, {10, 10}})));
  CommonAnyFluid.Components.ClosedVolumeColorP Vol3(redeclare package Medium = gas1, V = 0.01, nPorts = 2, valMin = pMinVis, valMax = pMaxVis) annotation(
    Placement(transformation(origin = {40, -34}, extent = {{-10, -10}, {10, 10}})));
equation
  ...
  connect(flowPath.port_b, Vol1.ports[1]) annotation(
  ...
  connect(BC_H_2.ports[1], Vol2.ports[1]) annotation(
    Line(points = {{-60, -44}, {-40, -44}}, color = {0, 127, 255}));
  connect(flowPath1.port_a, Vol2.ports[2]) annotation(
    Line(points = {{-20, -44}, {-40, -44}}, color = {0, 127, 255}));
  connect(flowPath1.port_b, Vol3.ports[1]) annotation(
    Line(points = {{20, -44}, {40, -44}}, color = {0, 127, 255}));
  connect(Vol3.ports[2], BC_L_2.ports[1]) annotation(
    Line(points = {{40, -44}, {60, -44}}, color = {0, 127, 255}));
  connect(BC_H_1.ports[1], Vol.ports[1]) annotation(
    Line(points = {{-60, 10}, {-40, 10}}, color = {0, 127, 255}));
  connect(Vol.ports[2], flowPath.port_a) annotation(
    Line(points = {{-40, 10}, {-20, 10}}, color = {0, 127, 255}));
protected
  annotation(
    experiment(StartTime = 0, StopTime = 20, Tolerance = 1e-06, Interval = 0.01));
end NozzleFlowEquation_IdealConstAir_ex01;
