...
model RectifierByTeslaValves_04
  ...
  CommonAnyFluid.Components.ClosedVolumeColorP volume6(redeclare package Medium = fluid1, V = 0.001, valMin = p_contour_min, valMax = p_contour_max, nPorts = 3) annotation(
    Placement(transformation(origin = {0, 56}, extent = {{-10, -10}, {10, 10}})));
  CommonAnyFluid.Components.ClosedVolumeColorP volume7(redeclare package Medium = fluid1, V = 0.001, valMax = p_contour_max, valMin = p_contour_min, nPorts = 3) annotation(
    Placement(transformation(origin = {0, -26}, extent = {{-10, -10}, {10, 10}})));
  CommonAnyFluid.Components.ClosedVolumeColorP volume(redeclare package Medium = fluid1, V = 0.001, nPorts = 3, valMax = p_contour_max, valMin = p_contour_min) annotation(
    Placement(transformation(origin = {-96, 10}, extent = {{-10, -10}, {10, 10}})));
  CommonAnyFluid.Components.ClosedVolumeColorP volume5(redeclare package Medium = fluid1, V = 0.001, nPorts = 3, valMax = p_contour_max, valMin = p_contour_min) annotation(
    Placement(transformation(origin = {96, 10}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Fluid.Pipes.DynamicPipe pipe(redeclare package Medium = fluid1, diameter = 0.1, length = 10) annotation(
    Placement(transformation(origin = {194, 14}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
equation
  ...
  connect(m_flow_sensor4.port_b, volume5.ports[3]) annotation(
    Line(points = {{90, -36}, {96, -36}, {96, 0}}, color = {0, 127, 255}));
  connect(m_flow_sensor6.port_b, pipe.port_a) annotation(
    Line(points = {{0, 100}, {194, 100}, {194, 24}}, color = {0, 127, 255}));
  connect(pipe.port_b, m_flow_sensor8.port_a) annotation(
    Line(points = {{194, 4}, {194, -20}}, color = {0, 127, 255}));
  annotation(
    experiment(StartTime = 0, StopTime = 50, Tolerance = 1e-05, Interval = 0.1),
    Diagram(coordinateSystem(extent = {{-200, -120}, {200, 120}})));
end RectifierByTeslaValves_04;