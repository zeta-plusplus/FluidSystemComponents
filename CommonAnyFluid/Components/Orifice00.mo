...
model Orifice00 "orifice with external input of Cd and area multiplier, incompressible equation"
  ...
  parameter Modelica.SIunits.Area AmechTh_paramInput = Modelica.Constants.pi / 4.0 * diam_paramInput ^ 2.0 "mechanical area of 'throat', valid if isCircular==false" annotation(
    Dialog(group = "Geometory"));
